...
class GameRepositoryTest {
    ...
    @Test
    fun `saveGame and loadActiveGame roundtrip with discard pile`() {
        val discardCards = listOf(SkyjoCard.NumberCard(10, 7))
        val state = GameState(
            phase = GamePhase.FINAL_TURNS,
            discardPile = DiscardPile(discardCards),
            finalTurnsRemaining = 2,
        )

        repo.saveGame("game-discard", state)
        val loaded = repo.loadActiveGame()

        assertNotNull(loaded)
        assertEquals(1, loaded!!.second.discardPile.size)
        assertEquals(2, loaded.second.finalTurnsRemaining)
    }

    @Test
    fun `savePlayerSession creates new session`() {
        repo.savePlayerSession("Alice", "game-1", connected = true)

        assertEquals("game-1", repo.getPlayerGame("Alice"))
    }

    @Test
    fun `savePlayerSession updates existing session`() {
        repo.savePlayerSession("Alice", "game-1", connected = true)
        repo.savePlayerSession("Alice", "game-2", connected = false)

        assertEquals("game-2", repo.getPlayerGame("Alice"))
    }
    ...
    fun `markDisconnected updates existing session`() {
        repo.savePlayerSession("Bob", "game-1", connected = true)
        repo.markDisconnected("Bob")

        assertEquals("game-1", repo.getPlayerGame("Bob"))
    }

    @Test
    fun `markDisconnected on unknown player does nothing`() {
        repo.markDisconnected("NoOne")
    }
